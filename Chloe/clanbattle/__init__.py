...
@on_command('add-clan', permission=perm.SUPERUSER | perm.GROUP_OWNER, shell_like=True, only_to_me=False)
...
async def del_member(session: CommandSession):
    parser = ArgumentParser(
        session=session, usage='del-member [--uid] [--alt]')
    parser.add_argument('--uid', type=int, default=-1)
    parser.add_argument('--alt', type=int, default=0)
    args = parser.parse_args(session.argv)
...
async def add_challenge(session: CommandSession):
    ...
    '''
    parser = ArgumentParser(
        session=session, usage='dmg -r -b damage [--uid] [--alt] [--ext | --last | --timeout]\n对3周目的老五造成114514点伤害：\n【 dmg 114514 -r3 -b5 】\n帮骑士A出了尾刀收了4周目带善人：\n【 dmg 1919810 -r4 -b1 --last --uid [骑士A的QQ] 】')
...
async def add_challenge_e(session: CommandSession):
    ...
    '''
    USAGE = "使用方法：\n【 dmge 伤害数字 r周目 几王 [last|ext|timeout] 】\n例：对5周目四王造成了114万点伤害\n【 dmge 114w r5 b4 】"
    challenge = session.state['challenge']
    challenge['uid'] = session.ctx['user_id']
    challenge['alt'] = 0
    flag = challenge['flag']
    f_cnt = (flag == BattleMaster.LAST) + \
        (flag == BattleMaster.EXT) + (flag == BattleMaster.TIMEOUT)
    if f_cnt > 1:
        # 此时应按ext处理，游戏内一刀不能获得两次补时
...
async def _(session: CommandSession):
    ...
    for arg in args:
        if rex_round.match(arg):
            ...
            ret['damage'] = 10000 * \
                int(arg[:-1]) if arg[-1] == 'w' or arg[-1] == 'W' else int(arg)
...
async def show_progress(session: CommandSession):
    ...
    round_, boss, remain_hp = battlemaster.get_challenge_progress(
        cid, datetime.now())
...
async def list_challenge(session: CommandSession):
    parser = ArgumentParser(
        session=session, usage='list-challenge [--cid] [--all]')
    parser.add_argument('--cid', type=int, default=1)
    # parser.add_argument('--uid', type=int, default=-1)
    # parser.add_argument('--alt', type=int, default=0)
    ...
    challen = battlemaster.list_challenge(
        cid, now) if args.all else battlemaster.list_challenge_of_day(cid, now, zone)
...
async def del_challenge(session: CommandSession):
    parser = ArgumentParser(
        session=session, usage='del-challenge --eid [--cid]')
