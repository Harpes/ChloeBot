...
async def _(session: CommandSession):
    ...
    await session.send(f'無羽R16-4[CQ:image,file=file:///{path}]')
...
async def _(session: CommandSession):
    ...
    await session.send(f'煌灵R16-4[CQ:image,file=file:///{path1}][CQ:image,file=file:///{path2}][CQ:image,file=file:///{path3}]')
